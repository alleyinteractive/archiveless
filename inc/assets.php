...
define( 'ARCHIVELESS_ASSET_MAP', read_asset_map( dirname( __DIR__ ) . '/build/assetMap.json' ) );
define( 'ARCHIVELESS_ASSET_MODE', ARCHIVELESS_ASSET_MAP['mode'] ?? 'production' );
...
function inline_locale_data( string $to_handle ) {
	...
	wp_add_inline_script(
		$to_handle,
		'wp.i18n.setLocaleData( ' . wp_json_encode( $locale_data ) . ", 'archiveless' );"
	);
}
